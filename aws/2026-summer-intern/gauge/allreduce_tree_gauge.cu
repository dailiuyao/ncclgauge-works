...
// POST_TO_DR split: POST_TO_RECVDONE = post → latest recv-done across children (waiting for
// incoming chunk to arrive), RECVDONE_TO_DR = recv-done → data ready (GPU reduce).
// Tree AllReduce: a non-leaf send at chunk i depends on the LATEST recv_done across
// all children on this channel at the SAME chunk index (no ring K-shift). Leaf ranks
// (no recv on this channel) print N/A for the split.
...
// For non-leaf chunks, POST_TO_RECVDONE / RECVDONE_TO_DR are gated on the SAME condition
// (so their denom equals Sum_valid_iters and the two means sum to mean(POST_TO_DR) exactly).
// For leaf ranks / channels without recv, the split is not accumulated
...
// Whether this (ch, i, k) accumulated the POST_TO_RECVDONE / RECVDONE_TO_DR split.
// Set on first accumulation; used at print time to decide N/A vs numeric.
...
static void ComputeIterativeOnet() {
    ...
    // Tree AllReduce on-wire chunk bytes: each rank sends the full message once
    // up-tree (reduce) and once down-tree (broadcast). Per-channel per-chunk send
    // volume: chunk_bytes = msg_bytes / (nch * nchunks_per_channel).
    ...
    double chunk_bytes = (double)msg_bytes / ((double)nch * ncpc);
...
void AccumulateTimingMetrics() {
    ...
    //
    // Tree AllReduce recv-split logic (POST_TO_RECVDONE / RECVDONE_TO_DR):
    //   A non-leaf send at chunk i depends on the LATEST recv_done across ALL
    //   children of this rank on this channel at the SAME chunk index. Reduce
    //   phase: sends to parent depend on recvs from every child. Broadcast
    //   phase: send to a child depends on recv from parent. Either way, per
    //   channel, the wait is on the max of recv_done stamps at chunk i.
    //
    // Recv peer discovery via net_Recv_posted_time (recv_peer_was_used). Tree
    // may have up to 2 recv peers per channel (binary tree). Root/leaf ranks
    // have no recv → recv split is N/A. Intra-node recv paths leave no NET
    // stamps → also N/A. Send-side samples still record in either case.
    for (size_t channelid = 0; channelid < (size_t)gauge_channels_num; channelid++) {
        if (!channel_was_used(channelid)) continue;

        // Collect all recv peers on this channel (tree: up to 2 children).
        int recv_peers[MAX_PEERS];
        int n_recv_peers = 0;
        for (int rk = 0; rk < MAX_PEERS && n_recv_peers < MAX_PEERS; ++rk) {
            if (recv_peer_was_used(channelid, rk)) {
                recv_peers[n_recv_peers++] = rk;
            }
        }
        ...
        for (size_t i = 0; i < std::min(static_cast<size_t>(gauge_chunk_num), static_cast<size_t>(MAX_GAUGE_CHUNKS)); ++i) {
            for (int k = 0; k < MAX_PEERS; ++k) {
                ...
                // POST_TO_RECVDONE / RECVDONE_TO_DR samples are possible from
                // proxy single-thread stamp jitter and pass through unchanged — use
                // median for a robust summary.

                // Tree recv split: take LATEST recv_done across all children at
                // chunk i on this channel. If no recv peers (leaf/root) or the
                // recv_done stamp is missing for every child this iter, leave
                // the split unset (print N/A). Send-side samples still record.
                Duration post_to_recvdone(0.0), recvdone_to_dr(0.0);
                bool this_iter_has_recv_split = false;
                if (n_recv_peers > 0) {
                    TimePoint recv_done_latest = TimePoint();
                    for (int rpi = 0; rpi < n_recv_peers; ++rpi) {
                        TimePoint t = recv_net_done_time[channelid][i][recv_peers[rpi]];
                        if (t.time_since_epoch().count() == 0) continue;
                        if (t > recv_done_latest) recv_done_latest = t;
                    }
                    if (recv_done_latest.time_since_epoch().count() != 0) {
                        // Raw split — no clamp. POST_TO_RECVDONE + RECVDONE_TO_DR == POST_TO_DR identically.
                        post_to_recvdone = Duration(recv_done_latest - post_i);
                        recvdone_to_dr = Duration(dr_i - recv_done_latest);
...
void printTimingMetrics() {
    ...
    printf("INFO: allreduce_tree_%s_rank(%d)_nranks(%d)_message_size(%s)_nchannels(%d)_nthreads(%s)_protocol(%s)_iteration(%d)\n",
    ...
    //
    // Printing scope: send/recv/rail/ctrl blocks below are printed for channel 0 ONLY
    // to keep the .out file small. K_EFF's O_net-window concurrency is still computed
    // over ALL channels (see ComputeIterativeOnet pool loop) — only the printout is
    // narrowed. Change `PRINT_CHANNEL_END = 1` to `gauge_channels_num` to restore
    // the full per-channel dump.
    const size_t PRINT_CHANNEL_END = std::min<size_t>(1, (size_t)gauge_channels_num);

    for (size_t channelid = 0; channelid < PRINT_CHANNEL_END; channelid++) {
        if (!channel_was_used(channelid)) continue;
        ...
        for (int k = 0; k < MAX_PEERS; ++k) {
            ...
            for (size_t i = 0; i < print_chunk_end(); ++i) {
                ...
                // POST_TO_RECVDONE / RECVDONE_TO_DR (recv-split gated)
                if (has_recv_split[channelid][i][k]) {
                    ...
                    printf("-- [POST_TO_RECVDONE] chunk %zu posted → recv done       : N/A (leaf/root or intra-node recv)\n", i);
                    printf("-- [RECVDONE_TO_DR] chunk %zu recv done → data ready   : N/A (leaf/root or intra-node recv)\n", i);
                }
...
int main(int argc, char* argv[]) {
    ...
    if (!env_gauge_mode_var) env_gauge_mode_var = "allreduce_tree";
    ...
    // GAUGE_STDOUT_ONLY_RANK0=1 -> only one rank writes a per-rank .out file;
    ...
    // when only a single rank's metrics are analyzed. Note this also drops
    // non-target-rank NCCL_DEBUG=INFO lines from the SLURM log.
    //
    // GAUGE_STDOUT_ONLY_RANK selects WHICH rank writes when the toggle is on
    // (default 0). E.g., in a Tree0/channel0 layout you may want rank 2 (0x7,
    // middle node) or rank 16 (0x0, middle node) instead of the root.
    const char* env_only_r0 = getenv("GAUGE_STDOUT_ONLY_RANK0");
    bool only_rank0 = (env_only_r0 && env_only_r0[0] == '1');
    int target_rank = 0;
    const char* env_target_rank = getenv("GAUGE_STDOUT_ONLY_RANK");
    if (env_target_rank && env_target_rank[0] != '\0') {
        target_rank = atoi(env_target_rank);
    }

    if (only_rank0 && myRank != target_rank) {
        ...
        char filename[256];
        sprintf(filename, "%s/nccl_allreduce_tree_%s_r-%d-%s.out",
