...
#SBATCH --nodelist=p6-odcr-queue-dy-p6b20048xlarge-[2,4,5,7]
#SBATCH --exclude=p6-odcr-queue-dy-p6b20048xlarge-[6,8]
#SBATCH -J nccl_test_4nodes_0x7
#SBATCH -o /home/liuyaod/netgauge-test/ncclguage/aws/2026-summer-intern/script/p6-b200/rough_roofline/%x-%j.out
#SBATCH -t 0:29:00
...
bash -c '
  # Per-rank GPU isolation: each rank sees only its own GPU. This avoids
  # a race where multiple ranks on the same node trigger CUDA primary-context
  # initialization on a shared device, then cudaSetDevice on the intended
  # device fails with "CUDA-capable device(s) is/are busy or unavailable".
  # CUDA_VISIBLE_DEVICES remaps the chosen GPU to logical device 0 inside
  # this rank, so we also tell nccl-tests to use device 0 (otherwise it
  # would compute localRank*nGpus+i and complain "Invalid number of GPUs").
