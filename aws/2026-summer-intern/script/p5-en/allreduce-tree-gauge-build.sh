...
#SBATCH -J allreduce-tree-gauge-build
...
for i in "${n_messages[@]}"; do
    nvcc $NVCC_GENCODE -ccbin g++ \
        ...
        "${GET_CYCLES_HOME}/get_clock.cu" "${GET_CYCLES_HOME}/allreduce_tree_gauge.cu" \
        -o "${GAUGE_OUT_DIR}/allreduce_tree_gauge_n_${i}.exe"
    if [ -f "${GAUGE_OUT_DIR}/allreduce_tree_gauge_n_${i}.exe" ]; then
        echo "Compilation successful: ${GAUGE_OUT_DIR}/allreduce_tree_gauge_n_${i}.exe"
    else
        echo "Compilation failed: allreduce_tree_gauge_n_${i}.exe"
        exit 1
    fi
done

echo "=== AllReduce Tree Gauge build complete for P5 ==="
