...
# GAUGE_STDOUT_ONLY_RANK selects WHICH rank writes (default 0). For 0x7 with
# ppn=1 on 4 nodes, Tree 0 / channel 0 lays out as root=0, middle=2, leaves=1,3;
# pick rank 2 (the middle node) so recv-split (POST_TO_RECVDONE) is populated.
export GAUGE_STDOUT_ONLY_RANK0=1
export GAUGE_STDOUT_ONLY_RANK=2
...
# Per-protocol message size: Simple=1GB, LL=8MB, LL128=128MB.
