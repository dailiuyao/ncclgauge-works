...
#SBATCH -J nccl_test_16nodes_topo_1G_16ch
...
msg_size="1G"
nch=16
for split_mask in 0x0 0x7; do
  for algo in Ring Tree; do
    for proto in Simple; do
      config_line="Config: NCCL_ALGO=${algo}, NCCL_PROTO=${proto}, NCCL_MIN_NCHANNELS=${nch}, NCCL_MAX_NCHANNELS=${nch}, msg_size=${msg_size}, SPLIT_MASK=${split_mask}"
      if echo "$done_configs" | grep -qxF "$config_line"; then
        echo ""
        echo "Skipping already-completed config: mask=${split_mask}/${algo}/${proto}/nch=${nch}/${msg_size}"
        continue
      fi
      ...
      bash -c '
        '"$nccl_tests/build/${nccl_test}_perf"' -b 1G -e 1G -f 2 -g 1 -c 1 -w 2 -n 5
      '
    done
  done
done
