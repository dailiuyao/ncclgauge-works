...
message_size() {
    if [ "$nch" -eq 4 ]; then
        ...
        declare -g MESSAGE_SIZE_LARGE_START=$((GAUGE_STEP_SIZE_LARGE * 64))
        declare -g MESSAGE_SIZE_LARGE_END=$((GAUGE_STEP_SIZE_LARGE * 1024))
        declare -g MESSAGE_SIZE_LARGE_STEP=$((GAUGE_STEP_SIZE_LARGE * 64))
        ...
        declare -g MESSAGE_SIZE_LARGE_START=$((GAUGE_STEP_SIZE_LARGE * 64 * 2))
        declare -g MESSAGE_SIZE_LARGE_END=$((GAUGE_STEP_SIZE_LARGE * 1024))
        declare -g MESSAGE_SIZE_LARGE_STEP=$((GAUGE_STEP_SIZE_LARGE * 64))
                ...
                for ((msize=${!START_VAR}; msize<=${!END_VAR}; msize+=${!STEP_VAR})); do
                    for mode in "${test_mode[@]}"; do
                        for ((nth = GAUGE_MIN_NTHREADS; nth <= GAUGE_MAX_NTHREADS; nth *= 2)); do
                            for combo in "0,0" "${d_number[@]},0"; do
                                ...
                                /opt/amazon/openmpi/bin/mpirun \
                                    ...
                                    $NCCL_GAUGE_HOME/gauge/${mode}_gauge_n_${n}.exe "$send_d" "$recv_d"
                            done
                        done
                    done
                done
