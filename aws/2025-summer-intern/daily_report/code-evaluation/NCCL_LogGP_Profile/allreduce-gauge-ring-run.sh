    ...
    for msg in "${msg_sizes[@]}"; do
        for ((iteration=0; iteration<GAUGE_NITERATIONS; iteration++)); do
            ...
            /opt/amazon/openmpi/bin/mpirun \
                ...
                $NCCL_GAUGE_HOME/gauge/${mode}_${NCCL_ALGO}_gauge_n_${N_ITERS}.exe
        done
    done
}

if [ "$NCCL_PROTO" = "Simple" ]; then
    run_gauge_test 1 "4/1024"
    run_gauge_test 1 $(for i in {1..5}; do echo $((2**i)); done)
    run_gauge_test 2 64
    run_gauge_test 4 128
    run_gauge_test 8 256 
    run_gauge_test 16 512
    run_gauge_test 16 $(seq 131072 131072 2097152)
elif [ "$NCCL_PROTO" = "LL" ]; then
    # run_gauge_test 1 "4/1024"
    # run_gauge_test 1 $(for i in {1..6}; do echo $((2**i)); done)
    run_gauge_test 1 128
    run_gauge_test 2 256
    run_gauge_test 4 512
    run_gauge_test 8 1024
    run_gauge_test 16 2048
    # run_gauge_test 16 $(seq 2048 1024 16384)
elif [ "$NCCL_PROTO" = "LL128" ]; then
    # run_gauge_test 1 "4/1024"
    run_gauge_test 1 $(for i in {1..2}; do echo $((2**i)); done)
    # run_gauge_test 1 8
    # run_gauge_test 2 16
    # run_gauge_test 4 32
    # run_gauge_test 8 64 
    # run_gauge_test 16 128
    # run_gauge_test 16 $(seq 16384 16384 262144)
fi
